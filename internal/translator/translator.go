...
func (t *Translator) AnalyzeSentiment(ctx context.Context, text string) (provider.SentimentResponse, error) {
	if t.provider == nil {
		client, err := t.createHTTPClient()
		if err != nil {
			return provider.SentimentResponse{}, fmt.Errorf("failed to create HTTP client: %w", err)
		}
		t.client = client

		providerCfg, ok := t.config.Providers[t.config.DefaultProvider]
		if !ok {
			return provider.SentimentResponse{}, fmt.Errorf("provider %s not configured", t.config.DefaultProvider)
		}

		p, err := provider.Get(t.config.DefaultProvider, providerCfg, t.client)
		if err != nil {
			return provider.SentimentResponse{}, fmt.Errorf("failed to initialize provider: %w", err)
		}
		t.provider = p
	}
...
func (t *Translator) ExtractTags(ctx context.Context, text string, count int) (provider.TagsResponse, error) {
	if t.provider == nil {
		client, err := t.createHTTPClient()
		if err != nil {
			return provider.TagsResponse{}, fmt.Errorf("failed to create HTTP client: %w", err)
		}
		t.client = client

		providerCfg, ok := t.config.Providers[t.config.DefaultProvider]
		if !ok {
			return provider.TagsResponse{}, fmt.Errorf("provider %s not configured", t.config.DefaultProvider)
		}

		p, err := provider.Get(t.config.DefaultProvider, providerCfg, t.client)
		if err != nil {
			return provider.TagsResponse{}, fmt.Errorf("failed to initialize provider: %w", err)
		}
		t.provider = p
	}
...
func (t *Translator) AnalyzeEmotions(ctx context.Context, text string) (provider.EmotionsResponse, error) {
	if t.provider == nil {
		client, err := t.createHTTPClient()
		if err != nil {
			return provider.EmotionsResponse{}, fmt.Errorf("failed to create HTTP client: %w", err)
		}
		t.client = client

		providerCfg, ok := t.config.Providers[t.config.DefaultProvider]
		if !ok {
			return provider.EmotionsResponse{}, fmt.Errorf("provider %s not configured", t.config.DefaultProvider)
		}

		p, err := provider.Get(t.config.DefaultProvider, providerCfg, t.client)
		if err != nil {
			return provider.EmotionsResponse{}, fmt.Errorf("failed to initialize provider: %w", err)
		}
		t.provider = p
	}
...
func (t *Translator) AnalyzeFactuality(ctx context.Context, text string) (provider.FactualityResponse, error) {
	if t.provider == nil {
		client, err := t.createHTTPClient()
		if err != nil {
			return provider.FactualityResponse{}, fmt.Errorf("failed to create HTTP client: %w", err)
		}
		t.client = client

		providerCfg, ok := t.config.Providers[t.config.DefaultProvider]
		if !ok {
			return provider.FactualityResponse{}, fmt.Errorf("provider %s not configured", t.config.DefaultProvider)
		}

		p, err := provider.Get(t.config.DefaultProvider, providerCfg, t.client)
		if err != nil {
			return provider.FactualityResponse{}, fmt.Errorf("failed to initialize provider: %w", err)
		}
		t.provider = p
	}
...
func (t *Translator) AnalyzeSensationalism(ctx context.Context, text string) (provider.SensationalismResponse, error) {
	if t.provider == nil {
		client, err := t.createHTTPClient()
		if err != nil {
			return provider.SensationalismResponse{}, fmt.Errorf("failed to create HTTP client: %w", err)
		}
		t.client = client

		providerCfg, ok := t.config.Providers[t.config.DefaultProvider]
		if !ok {
			return provider.SensationalismResponse{}, fmt.Errorf("provider %s not configured", t.config.DefaultProvider)
		}

		p, err := provider.Get(t.config.DefaultProvider, providerCfg, t.client)
		if err != nil {
			return provider.SensationalismResponse{}, fmt.Errorf("failed to initialize provider: %w", err)
		}
		t.provider = p
	}
